(function () {
  ...
  const DEFAULT_QA = [
    ...
    },
    {
      "__id": "qa_2",
      "category": "delivery",
      "question": "독일 본사 제품을 주문했는데 수입신고필증에 적출국이 네델란드 혹은 벨기에인 이유는 무엇인가요?",
      "keywords": ["적출국", "네덜란드", "벨기에", "독일", "수입신고필증"],
    ...
    },
    {
      "__id": "qa_4",
      "category": "documents",
      "question": "원산지 증명서를 생산자인 윌로가 아닌 고객측에서 발급받을 시 제조공정도와 BOM 자재명세서를 발급자에게 직접 제출할 수 있나요?",
      "keywords": ["원산지증명서", "BOM", "제조공정도", "자재명세서"],
      "answer": "생산자가 직접 원산지증명서를 발급하지 않고 수출자가 발급받는 경우,\n수출자에게 BOM 자재명세서, 제조공정도, 원산지 소명서 등의 자료를 직접 제출하는 것은 어렵습니다.\n\n이는 해당 자료가 회사 내부 정보 관리상 단가, 공급업체 정보 등 대외비에 해당하기 때문이며,\n발급 기관에서 BACK DATA로 요청하는 자료(제조공정도, BOM 자재명세서, 원산지 소명서 등)는 발급 신청자에게가 아닌, 발급 기관(대한상공회의소 또는 세관)에 생산자가 직접 제출하는 방식으로 진행합니다.\n\n따라서, 증명서를 신청하는 주체는 대한상공회의소 원산지증명센터 또는 세관에 원산지증명서 발급 접수 후 접수번호를 회신해 주시기 바라며, 생산자인 윌로는 해당 접수번호를 기준으로 발급 기관에 직접 관련 자료를 제출하여 대외비 노출 리스크 없이 안전하게 발급 절차를 진행합니다.",
      "top": false
    },
    {
      "__id": "qa_5",
    ...
    },
    {
      ...
      "keywords": ["원산지확인서", "원산지증명서", "차이", "포괄확인서"],
    ...
    },
    {
      ...
      "keywords": ["성적서", "Mill Certificate", "Mill Sheet", "자재"],
  ...
  function validateItem(item) {
    ...
    if (!item) {
      errors.push("항목이 비어 있습니다.");
      return errors;
    }

    if (!item.question || item.question.length < 3) {
      errors.push("질문은 최소 3자 이상이어야 합니다.");
    }

    if (item.question && item.question.length > MAX_QUESTION_LENGTH) {
      errors.push(`질문은 ${MAX_QUESTION_LENGTH}자를 초과할 수 없습니다.`);
    }

    if (!item.answer || item.answer.length < 5) {
      errors.push("답변은 최소 5자 이상이어야 합니다.");
    }

    if (item.answer && item.answer.length > MAX_ANSWER_LENGTH) {
      errors.push(`답변은 ${MAX_ANSWER_LENGTH}자를 초과할 수 없습니다.`);
    }

    if (!item.keywords || item.keywords.length === 0) {
      errors.push("최소 1개 이상의 키워드가 필요합니다.");
    }
  ...
  function ensureLatestDefaultQA() {
    const currentVersion = safeGetItem(QA_VERSION_KEY, null);
    const stored = safeParse(safeGetItem(QA_KEY, null), null);

    if (currentVersion !== QA_VERSION || !Array.isArray(stored) || !stored.length) {
      const normalizedDefaults = DEFAULT_QA
        .map(normalizeItem)
        .filter(item => item !== null);
      
      safeSetItem(QA_KEY, JSON.stringify(normalizedDefaults));
      safeSetItem(QA_VERSION_KEY, QA_VERSION);
  ...
  function saveQA(items) {
    ...
    const success = safeSetItem(QA_KEY, JSON.stringify(normalized));
    if (success) {
      safeSetItem(QA_VERSION_KEY, QA_VERSION);
    }
    return success;
  ...
  // ========== History 관리 ==========
  function getHistory() {
    const data = safeParse(safeGetItem(HISTORY_KEY, "[]"), []);
  ...
  function saveHistoryItem(item) {
    ...
    return safeSetItem(HISTORY_KEY, JSON.stringify(history.slice(0, MAX_HISTORY_SIZE)));
  }
  ...
  function matchesQuestion(item, text) {
    ...
    if (calculateSimilarity(q, itemQuestion) >= 0.8) return true;

    if (itemQuestion && q.includes(itemQuestion)) return true;
    if (itemQuestion && itemQuestion.includes(q)) return true;

    return item.keywords.some(keyword => {
      const normalizedKeyword = normalizeText(keyword);
      return q.includes(normalizedKeyword) || normalizedKeyword.includes(q);
    });
  }
  ...
  window.ChatbotStore = {
    ...
    findAnswer(text, selectedCategory) {
      ...
      const sanitizedText = sanitizeString(text, MAX_QUESTION_LENGTH);
      const candidates = this.getQAByCategory(selectedCategory === "all" ? "all" : selectedCategory);
      
      const found = candidates.find(item => matchesQuestion(item, sanitizedText));
      if (found) return found;

      if (selectedCategory !== "all") {
        return loadQA().find(item => matchesQuestion(item, sanitizedText)) || null;
      }
    ...
    importTemplate(payload) {
      ...
      const incoming = Array.isArray(payload.data) 
        ? payload.data.map(normalizeItem).filter(item => item !== null)
        : [];

      if (mode === "append") {
        const current = loadQA();
        return saveQA(current.concat(incoming));
      } else {
        return saveQA(incoming);
      }
    },
    ...
    deleteQAByIndex(index) {
      ...
      return saveQA(current);
    },

    clearQA() {
      safeSetItem(QA_KEY, JSON.stringify([]));
      safeSetItem(QA_VERSION_KEY, QA_VERSION);
      return true;
    },
    ...
    validateItem(item) {
      ...
      if (!normalized) {
        return { valid: false, errors: ['항목 정규화에 실패했습니다.'] };
      }
    ...
    getStats() {
      const qa = loadQA();
      const history = getHistory();
      const categories = this.getCategories().filter(c => c !== 'all');

      return {
        qaCount: qa.length,
        historyCount: history.length,
        matchedCount: history.filter(h => h.matched).length,
        categories: categories.map(cat => ({
          name: cat,
          label: CATEGORY_LABELS[cat],
          count: qa.filter(item => item.category === cat).length
        })),
        topQuestionsCount: qa.filter(item => item.top).length
      };
    },
    ...
    importData(data) {
      ...
      try {
        ...
        if (Array.isArray(data.history)) {
          safeSetItem(HISTORY_KEY, JSON.stringify(data.history.slice(0, MAX_HISTORY_SIZE)));
        }
